...
def statistic_analysis (data):
    ...
    def bar_pie_plot2(data):
        if data.columns[1] == "sentiment polarity class":
            ...
            xtickSize = 8
            ytickSize = 10
        elif data.columns[1] == "sentiment polarity level":
        ...
        def addlabels(x,y,z):
            for i in range(len(x)):
                label = str(y[i])+ '\n' + str(round((z[i]*100),2))+'%'
                plt.text(i, y[i]//2, label, ha = 'center', fontsize=14)
...
data = statistic_analysis(data)
# data.to_csv(r'C:\Users\AlanKoo99\Desktop\test.csv', index = False)
