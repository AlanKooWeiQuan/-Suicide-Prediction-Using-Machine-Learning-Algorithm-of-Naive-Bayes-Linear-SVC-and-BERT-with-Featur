...
data2 = data_loading.load_data(r'C:\Users\AlanKoo99\Desktop\FYP coding\data\testing\1_kaggle\kaggle_testing_data_emotion.csv')
data3 = model_20_testing(data, data2)
data3 = data3.drop(data3.columns[[0]],axis = 1)
data3.to_csv(r'C:\Users\AlanKoo99\Desktop\FYP coding\data\testing\1_kaggle\kaggle_testing_data_prediction_result.csv')

#### print result
for n in range(len(data3)):
# for n in range(20):
    sys.stdout.buffer.write(str(n).encode("utf-8") + b'. ' + data3.ori_text.iloc[n].encode("utf-8") + b'\n')
    # print(str(n) + ". " + data3.ori_text.iloc[n].encode("utf-8") + '\n')
...
print("total text data: " + str(len(data3)))
matchs = 0
notmatchs = 0
for n in range (len(data3)):
    if data3['class'].iloc[n] == data3['actual_class'].iloc[n]:
        matchs +=1
    elif data3['class'].iloc[n] != data3['actual_class'].iloc[n]:
        notmatchs +=1
print("match: " + str(matchs))
print("notmatch: " + str(notmatchs))
print("accuracy = " + str((matchs/len(data3))))
